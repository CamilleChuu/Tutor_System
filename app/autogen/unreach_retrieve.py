...
def count_unreach_records(collection, session_id='0', limit = 3):
    results = collection.find({'session': session_id, 'speaker': 'user'}, sort=[('_id', -1)])
    last_record_satisfies_condition = True
    exp_set = collection.find_one({'session': session_id, 'speaker': 'user'}, sort=[('_id', -1)])['reached']
    i = 0
    for item in results:
        if item['reached'] == exp_set: 
            i += 1
        if i>limit: return 1
    return 0