...
class GroupChatExp:
    ...
    speaker_selection_method: Optional[str] = "exp_round"
    allow_repeat_speaker: Optional[Union[bool, List[Agent]]] = True

    _VALID_SPEAKER_SELECTION_METHODS = ["auto", "manual", "random", "round_robin", "exp_round"]
...
Then select the next role from {[agent.name for agent in agents]} to play. Only return the role."""
    ...
    def _prepare_and_select_agents(
        ...
        elif self.speaker_selection_method.lower() == "exp_round":
            selected_agent = self.next_exp_agent(last_speaker, agents)
        else:
            selected_agent = None
...
class GroupChatManagerExp(ConversableWritableAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.register_reply(Agent, GroupChatManagerExp.run_chat, config=groupchat, reset_config=GroupChatExp.reset)
        # Allow async chat if initiated using a_initiate_chat
        self.register_reply(
            Agent,
            GroupChatManagerExp.a_run_chat,
            config=groupchat,
            reset_config=GroupChatExp.reset,
    ...
    def run_chat(
        ...
        config: Optional[GroupChatExp] = None,
    ) -> Union[str, Dict, None]:
        """Run a group chat."""
        if messages is None:
            ...
            for agent in groupchat.agents:
                if agent != speaker:
                    if not agent.name == 'Administrator':
                        if not (agent.__class__==speaker.__class__ and 'AssistantExpAgent' in str(speaker.__class__)):
