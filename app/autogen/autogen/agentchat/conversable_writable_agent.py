...
__all__ = ("ConversableWritableAgent",)
...
class ConversableWritableAgent(Agent):
    ...
    def __init__(
        ...
        self._is_termination_msg = (
            ...
            else (lambda x: "TERMINATE" in content_str(x.get("content"))[-10:])
        )
        self.writedown = writedown,
        self.writedir = writedir,
        self.writemd = writemd,
        self.writesign = writesign,
        ...
        self.register_reply([Agent, None], ConversableWritableAgent.generate_oai_reply)
        self.register_reply([Agent, None], ConversableWritableAgent.a_generate_oai_reply, ignore_async_in_sync_chat=True)
        self.register_reply([Agent, None], ConversableWritableAgent.generate_code_execution_reply)
        self.register_reply([Agent, None], ConversableWritableAgent.generate_tool_calls_reply)
        self.register_reply([Agent, None], ConversableWritableAgent.a_generate_tool_calls_reply, ignore_async_in_sync_chat=True)
        self.register_reply([Agent, None], ConversableWritableAgent.generate_function_call_reply)
        self.register_reply(
            [Agent, None], ConversableWritableAgent.a_generate_function_call_reply, ignore_async_in_sync_chat=True
        )
        self.register_reply([Agent, None], ConversableWritableAgent.check_termination_and_human_reply)
        self.register_reply(
            [Agent, None], ConversableWritableAgent.a_check_termination_and_human_reply, ignore_async_in_sync_chat=True
        )
    ...
    def register_reply(
        ...
        *,
        ...
        chats (initiated with `ConversableWritableAgent.a_initiate_chat`). If an `async` reply function is registered
        ...
        ```python
        def reply_func(
            recipient: ConversableWritableAgent,
    ...
    def _print_received_message(self, message: Union[Dict, str], sender: Agent):
        ...
        print(colored(sender.name, "yellow"), "(to2", f"{self.name}):\n", flush=True)
        message = self._message_to_dict(message)
        ...
        if message.get("role") in ["function", "tool"]:
            ...
            if content is not None:
                ...
                if self.writedown:
                    if self.writesign[0] in re.sub('\s', '', str(content)):
                        with open(self.writedir[0], self.writemd[0]) as f:  pickle.dump(content, f)
    ...
    def _prepare_chat(self, recipient: "ConversableWritableAgent", clear_history: bool, prepare_recipient: bool = True) -> None:
    ...
    def _raise_exception_on_async_reply_functions(self) -> None:
        ...
        if async_reply_functions != []:
            msg = (
                "Async reply functions can only be used with ConversableWritableAgent.a_initiate_chat(). The following async reply functions are found: "
    ...
    def register_hook(self, hookable_method: Callable, hook: Callable):
        ...
        Args:
            hookable_method: A hookable method implemented by ConversableWritableAgent.
