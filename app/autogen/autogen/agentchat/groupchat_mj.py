...
class GroupChatMJ:
    ...
    - speaker_selection_method: the method for selecting the next speaker. Default is "auto".
        ...
        - "exp_round": my setting, individual given infor, combine after critic;
    ...
    speaker_selection_method: Optional[str] = "vote_round"
    allow_repeat_speaker: Optional[Union[bool, List[Agent]]] = True

    _VALID_SPEAKER_SELECTION_METHODS = ["auto", "manual", "random", "round_robin", "vote_round"], 
    ...
    def next_mj_agent(self, agent: Agent, agents: Optional[List[Agent]] = None) -> Agent:
        """Return the next agent in the list."""  ## no change to robin round
        if agents is None:
            agents = self.agents
...
Then select the next role from {[agent.name for agent in agents]} to play. Only return the role."""
    ...
    def _prepare_and_select_agents(
        ...
        elif self.speaker_selection_method.lower() == "vote_round":
            selected_agent = self.next_mj_agent(last_speaker, agents)
        else:
            selected_agent = None
    ...
    def select_speaker(self, last_speaker: Agent, selector: ConversableWritableAgent):
    ...
    async def a_select_speaker(self, last_speaker: Agent, selector: ConversableWritableAgent):
        """Select the next speaker."""
        selected_agent, agents, messages = self._prepare_and_select_agents(last_speaker)
        if selected_agent:
            return selected_agent
...
class GroupChatManagerMJ(ConversableWritableAgent):
    """(In preview) A chat manager agent that can manage a group chat of multiple agents."""

    def __init__(
        self,
        groupchat: GroupChatMJ,
        ...
        **kwargs,
        ...
        self.register_reply(Agent, GroupChatManagerMJ.run_chat, config=groupchat, reset_config=GroupChatMJ.reset)
        # Allow async chat if initiated using a_initiate_chat
        self.register_reply(
            Agent,
            GroupChatManagerMJ.a_run_chat,
            config=groupchat,
            reset_config=GroupChatMJ.reset,
    ...
    def _prepare_chat(self, recipient: ConversableWritableAgent, clear_history: bool, prepare_recipient: bool = True) -> None:
        ...
        for agent in self._groupchat.agents:
            if (recipient != agent or prepare_recipient) and isinstance(agent, ConversableWritableAgent):
    ...
    def run_chat(
        ...
        config: Optional[GroupChatMJ] = None,
    ) -> Union[str, Dict, None]:
        """Run a group chat."""
        if messages is None:
        ...
        for i in range(groupchat.max_round):
            ...
            for agent in groupchat.agents:
                if agent != speaker:
                    if not agent.name == 'Administrator':
                        if  (agent.__class__==speaker.__class__ and 'AssistantExpAgent' in str(speaker.__class__)):
                            continue # no connection between judgers
                        elif ('Controller' in str(agent.name) or 'Half' in str(agent.name)  ) and ('Majority' in str(speaker.name)):
                            continue # no information from majority- to half- critic or to controller
                        else:
